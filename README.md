...
基于 RK3588/RK3568 NPU 的工业标签缺陷检测系统，支持 USB/CSI 摄像头、GPIO 外部触发、MQTT 数据上报。
...
| 项目 | 要求 |
|------|------|
| 开发板 | RK3588 或 RK3568（如 ELF 2 开发板） |
| NPU | RKNN 运行时（librknnrt） |
| 摄像头 | USB 摄像头（/dev/video21）或 MIPI-CSI（OV13855） |
| 触发 IO | GPIO 139（可通过设备树配置） |
